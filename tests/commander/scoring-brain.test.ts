...
describe("ScoringBrain", () => {
  ...
  test("returns empty assignments for empty fleet", async () => {
    const result = await brain.evaluate({
      fleet: makeFleet([]),
      goals: [],
  ...
  test("assigns idle bots (ready status) to routines", async () => {
  ...
  test("does not reassign bots already on optimal routine (no improvement)", async () => {
  ...
  test("maximize_income shifts assignments toward trader/miner", async () => {
    const bot = makeBot({ botId: "bot1", status: "ready", routine: null });
  ...
  test("prepare_for_war shifts assignments toward hunter", async () => {
    const bot = makeBot({ botId: "bot1", status: "ready", routine: null });
  ...
  test("supply deficits boost relevant routines", async () => {
    const bot = makeBot({ botId: "bot1", status: "ready", routine: null });
  ...
  test("switch cost penalizes role changes", async () => {
  ...
  test("docked bots have lower switch cost", async () => {
  ...
  test("diversity penalty kicks in when too many bots on same routine", async () => {
    const bots = [
      makeBot({ botId: "bot1", routine: "miner" }),
      makeBot({ botId: "bot2", routine: "miner" }),
      makeBot({ botId: "bot3", routine: "miner" }),
      makeBot({ botId: "bot4", routine: "miner" }),
      makeBot({ botId: "bot5", status: "ready", routine: null }),
    ];
    const fleet = makeFleet(bots);
    const weights = { miner: 1, harvester: 1, trader: 1, explorer: 1, crafter: 1, hunter: 1, salvager: 1, mission_runner: 1, return_home: 0.1, scout: 0.1 };

    // Scoring miner for bot5 should have diversity penalty (4 miners already, threshold=4)
    const minerScore = brain.scoreAssignment(bots[4], "miner", weights, emptyEconomy, fleet);
    const traderScore = brain.scoreAssignment(bots[4], "trader", weights, emptyEconomy, fleet);
  ...
  test("low fuel bots penalized for most routines", async () => {
  ...
  test("respects reassignment cooldown", async () => {
    ...
    const result2 = await brain2.evaluate({
      fleet: makeFleet([bot]),
  ...
  test("clearCooldown allows immediate reassignment", async () => {
    const brain2 = new ScoringBrain({ reassignmentCooldownMs: 60_000 });
    const bot = makeBot({ botId: "bot1", status: "ready", routine: null });

    await brain2.evaluate({
      fleet: makeFleet([bot]),
      goals: [],
      economy: emptyEconomy,
      tick: 1,
  ...
  test("reasoning includes goal and assignment summary", async () => {
    const bot = makeBot({ botId: "bot1", status: "ready", routine: null });
  ...
  test("updateConfig changes scoring behavior", async () => {
    ...
    const result2 = await brain.evaluate({
      fleet: makeFleet([bot]),
      goals: [],
      economy: emptyEconomy,
      tick: 2,
  ...
  test("assigns different routines to multiple bots", async () => {
    const bots = [
